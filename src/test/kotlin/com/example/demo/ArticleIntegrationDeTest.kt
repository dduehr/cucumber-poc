...
class ArticleIntegrationDeTest {
    @LocalServerPort
    private var port: Int? = 0

    @Before
    fun setup(scenario: Scenario) {
        println("## Szenario: ${scenario.name} Ausgeführt in: ${Thread.currentThread().name}")
    ...
    fun step(scenario: Scenario) {
        println("## Szenario: ${scenario.name} Schritte ausgeführt in: ${Thread.currentThread().name}")
    }

    @Given("Erstelle einen Artikel mit folgenden Feldern")
    ...
    @Then("Sollte die Antwort den Status {int} haben")
